...
from utils import *
from run_utils_nora_d import *
from lora import run_nora_d, run_warmup_asvd

def main(config=None):

    # Load config file and set noRA_D parameters
    args = get_arguments()

    # Load wandb.agent hy-config
    wandb.init(config=config, name='NoRA_D', notes="表D: 验证在设置W_a为LoRA后，额外加wb 和out的向量有额外提升，介于nora的独特特性，需要增大W_a的内部r")
    config = wandb.config
    args.dataset = config.dataset
    args.init_method = config.init_method
    args.Wbout_structure = config.Wbout_structure
    ...
    run_nora_d(args, clip_model, logit_scale, dataset, train_loader, val_loader, test_loader, text_W_prime_dict, text_S_dict, vision_W_prime_dict, vision_S_dict)
