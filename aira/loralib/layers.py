...
class MultiheadAttentionAwLoRA(nn.Module):
    def __init__(
            self,
            existing_mha: nn.MultiheadAttention,
            enable_lora: list = ['q', 'k', 'v', 'o'],
            ranks: dict = None,  # 新增ranks参数，用于存储每个投影层的rank值
            lora_alpha: int = 1, 
            dropout_rate: float = 0.,
            svd_lora: bool = False,
            svd_step: int = 1,
        ...
        # 如果没有提供ranks，则使用默认值0
        ranks = ranks or {'q': 0, 'k': 0, 'v': 0, 'o': 0}
        ...
        LoRALayer.__init__(self, r=max(ranks.values()), lora_alpha=lora_alpha, dropout_rate=dropout_rate)
        
        # Init qkv as a new lora linear layer 
        for item in enable_lora:
            if item == 'q':
                self.q_proj = AwLinear(self.q_proj,
                                       r=ranks['q'],
                                       lora_alpha=lora_alpha,
                                       fan_in_fan_out=False,
                                       dropout_rate=dropout_rate,
                ...
                self.k_proj = AwLinear(self.k_proj,
                                       r=ranks['k'],
                                       lora_alpha=lora_alpha,
                                       fan_in_fan_out=False,
                                       dropout_rate=dropout_rate,
                ...
                self.v_proj = AwLinear(self.v_proj,
                                       r=ranks['v'],
                                       lora_alpha=lora_alpha,
                                       fan_in_fan_out=False,
                                       dropout_rate=dropout_rate,
                ...
                self.o_proj = AwLinear(self.o_proj,
                                       r=ranks['o'],
                                       lora_alpha=lora_alpha,
                                       fan_in_fan_out=False,
                                       dropout_rate=dropout_rate,
